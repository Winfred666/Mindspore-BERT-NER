<template>
	<div :id='"charts-content"+id_postfix' class=" w-80 h-48"></div>
</template>
...
const series = merge({}, defaultSeries[0]) // {}表示合并后的新对象，可以传入一个空对象作为初始值

series.data = props.seriesData

function DrawWordCloud() {
	// 词云
	let mychart = echarts.init(document.getElementById("charts-content"+props.id_postfix)) // 可以设置主题色'dark'
	mychart.setOption({
		series: series
	})
}
